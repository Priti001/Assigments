...
select f.title, count(a.actor_id) as No_of_actor from film f
join film_actor a
on f.film_id = a.film_id
group by f.title;

#6d. How many copies of the film Hunchback Impossible exist in the inventory system?
select f.title,count(i.inventory_id) as Inventory from inventory i
join film f
on i.film_id = f.film_id
where f.title  = 'Hunchback Impossible';

#6e. Using the tables payment and customer and the JOIN command, list the total paid by each customer. List the customers alphabetically by last name:
select c.last_name,p.amount from payment p
join customer c
...
select s.store_id as Store, p.staff_id as Staff,sum(p.amount) as Business
from payment p
join staff s
...
select s.store_id,c.city,cu.country from store s
join address a
on s.address_id = a.address_id
join city c
